...
namespace LogicAnalyzer
{
    public partial class MainWindow : PersistableWindowBase
    {
        ...
        PreviewSettings? lastPreviewSettings;
        ...
        {
            if (session == null || e.Position > session.TotalSamples)
                return;
        ...
        private async void MnuNetSettings_Click(object? sender, RoutedEventArgs e)
        {
            ...
            if (await dlg.ShowDialog<bool>(this))
            {
                ...
                try
                {
                    res = driver.SendNetworkConfig(dlg.AccessPoint, dlg.Password, dlg.Address, dlg.Port, dlg.Hostname);
                }
                catch { }
        ...
        private async void btnRealtime_Click(object? sender, RoutedEventArgs e)
        {
            ...
            var dlg = new Dialogs.PreviewDialog();
            dlg.Initialize(driver, lastPreviewSettings);
            var result = await dlg.ShowDialog<bool?>(this);

            if (result != true || dlg.SelectedSettings == null)
                return;

            StartPreview(dlg.SelectedSettings);
        }

        private void StartPreview(PreviewSettings settings)
        {
            if (driver == null)
                return;
            ...
            driver.PreviewDataReceived += Driver_PreviewDataReceived;

            bool started = driver.StartRealtimePreview(
                settings.Channels,
                settings.IntervalsPerSecond,
                settings.SamplesPerInterval);

            if (!started)
            {
                driver.PreviewDataReceived -= Driver_PreviewDataReceived;
                _ = this.ShowError("Error", "Failed to start realtime preview.");
                return;
            }
            ...
            previewViewer.IsVisible = true;
            grdContent.IsVisible = false;
            grdChannelHeader.IsVisible = false;
            scrSamplePos.IsVisible = false;
            samplePreviewer.IsVisible = false;

            previewViewer.Initialize(settings.Channels, settings.MaxDisplaySamples);
        ...
        private void StopPreview()
        {
            ...
            {
                driver.PreviewDataReceived -= Driver_PreviewDataReceived;
                driver.StopRealtimePreview();
            }

            isPreviewActive = false;

            // Restore normal UI
            previewViewer.IsVisible = false;
            grdContent.IsVisible = true;
            grdChannelHeader.IsVisible = true;
            scrSamplePos.IsVisible = true;

            syncUI();
        }

        private void Driver_PreviewDataReceived(object? sender, PreviewEventArgs e)
        {
            Dispatcher.UIThread.Post(() =>
            {
                previewViewer.AddSamples(e.Samples);
            });
        }
        ...
        private void syncUI()
        {
            ...
            bool canCapture = hasDriver && !isCapturing && !isPreviewActive;
            ...
            bool canPreview = hasDriver && !isCapturing && driver!.DriverType != AnalyzerDriverType.Multi;

            btnOpenClose.IsEnabled = !isCapturing && !isPreviewActive;
            ...
            btnRealtime.IsEnabled = canPreview;
            btnRealtime.Content = isPreviewActive ? "Stop" : "Realtime";
